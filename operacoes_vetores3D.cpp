...
// Faz o operador do cout "<<", fazer output de um vetor no formato "[x, y, z]" (Operator overloading)
...
int main(void)
{
    ...
    while (execute_operacao)
    {
        cout << "Qual operação deseja efetuar?? (Digite o número correspondente)\n"
             ...
             << "5 - Produto vetorial\n";
        cin >> input_operacao;
        // Checa input da operação e repete o input caso o erro persista
        ...
        // Pergunta ao usuário se ele quer fazer alguma outra operação
        cout << "Deseja fazer outra operação com os mesmos vetores??" << endl
             << "(Digite 's' para sim ou qualquer outra tecla para terminar o programa)" << endl;
        cin >> resposta_usuario;
        if (tolower(resposta_usuario) != 's')
        {
